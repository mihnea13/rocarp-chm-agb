...
fingerprint, a fold or a poorly printed edge.
    ...
    pip install segno
    python tools/make_qr.py https://GITHUB-USER.github.io/rocarp-chm-agb/
...
DEFAULT_URL = "https://GITHUB-USER.github.io/rocarp-chm-agb/"


def main() -> None:
    url = sys.argv[1] if len(sys.argv) > 1 else DEFAULT_URL
    if "GITHUB-USER" in url:
        sys.exit("Replace GITHUB-USER with the real account before generating the code.")
    ...
    # Raster fallback: ~30 mm at 600 dpi.
    qr.save(out / "rocarp-qr.png", scale=30, border=4, dark="#000000", light="#ffffff")
